...
def _is_cron_approval_context() -> bool:
    """True when this call is inside a cron execution context.

    Prefer the context-local cron marker when available so a leaked
    process-global ``HERMES_CRON_SESSION`` from a prior scheduler tick cannot
    misclassify later gateway turns in the same Python process.
    """
    try:
        from gateway.session_context import get_session_env
